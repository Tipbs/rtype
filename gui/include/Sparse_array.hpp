...
template<typename Component> // You can also mirror the definition of
                             // std::vector, that takes an additional allocator.
class sparse_array {
  public:
    ...
    using container_t =
        std::vector<value_type>; // optionally add your allocator template here.
  ...
  public:
    ...
    sparse_array &
    operator=(sparse_array const &) = default; // copy assignment operator
    sparse_array &
    operator=(sparse_array &&) noexcept = default; // move assignment operator
...
template<typename Component>
inline typename sparse_array<Component>::const_reference_type
...
template<typename Component>
inline typename sparse_array<Component>::iterator
sparse_array<Component>::begin()
{
    return _data.begin();
}

template<typename Component>
inline typename sparse_array<Component>::const_iterator
...
template<typename Component>
inline typename sparse_array<Component>::const_iterator
sparse_array<Component>::end() const
{
    return _data.end();
}
...
template<typename Component>
inline typename sparse_array<Component>::size_type
sparse_array<Component>::size() const
{
    return _data.size();
}
 ...
 */
template<typename Component>
inline sparse_array<Component>::reference_type
sparse_array<Component>::add_entity(size_type pos)
{
    if (pos > this->size())
        pos = this->size();
    if (pos < this->size())
...
template<typename Component>
inline typename sparse_array<Component>::size_type
sparse_array<Component>::get_index(value_type const &comp) const
{
    auto it = 0;
    while (it < size()) {
        if (_data[it] == comp)
