    ...
    sparse_array<Direction> &dir)
{
    for (size_t ind = 0; ind < positions.size(); ind++) {
        ...
        if (!(pos && spe && diro))
            continue;
        std::cout << "BEFORE : pos : " << positions[ind].value().pos_X << ", "
                  << positions[ind].value().pos_Y
                  << "\nspeed : " << speed[ind].value().speed
                  << "\ndir : " << dir[ind].value().dir_X << ", "
                  << dir[ind].value().dir_Y << std::endl
                  << std::endl;
        double magnitude = std::sqrt(
            (dir[ind].value().dir_X * dir[ind].value().dir_X) +
            (dir[ind].value().dir_Y * dir[ind].value().dir_Y));
        std::cout << "magnitude " << magnitude << std::endl;
        if (magnitude > 0.1) { // Added a magnitude threshold to avoid going
                               // straight to INT_MIN and INT_MAX when having a
                               // really low direction move
        ...
        std::cout << "AFTER : pos : " << positions[ind].value().pos_X << ", "
                  << positions[ind].value().pos_Y
                  << "\nspeed : " << speed[ind].value().speed
                  << "\ndir : " << dir[ind].value().dir_X << ", "
                  << dir[ind].value().dir_Y << std::endl
    ...
    sparse_array<Sprite> &sprite)
{
    ...
    for (size_t ind = 0; ind < sprite.size(); ind++) {
        ...
        if (!(pos && siz && spri))
            continue;
        DrawTexture(
            sprite[ind].value().sprite, positions[ind].value().pos_X,
            positions[ind].value().pos_Y, WHITE);
    }
    EndDrawing();
}
    ...
    sparse_array<SpawnGrace> &grace)
{
    ...
    for (size_t ind = 0; ind < positions.size(); ind++) {
        ...
        std::cout << "temps d'origine : "
                  << grace[ind].value_or(SpawnGrace(0, 0)).creation_time
                  << std::endl;
        std::cout << "temps de grace : "
                  << grace[ind].value_or(SpawnGrace(0, 0)).timer << std::endl;
        std::cout << "temps actuel : " << time << std::endl;
        if (grace[ind].value_or(SpawnGrace(0, 0)).creation_time +
                grace[ind].value_or(SpawnGrace(0, 0)).timer >=
            time)
            continue;
        for (size_t ind2 = ind + 1; ind2 < positions.size(); ind2++) {
            if (grace[ind2].value_or(SpawnGrace(0, 0)).creation_time +
                    grace[ind2].value_or(SpawnGrace(0, 0)).timer >=
                time)
                continue;
